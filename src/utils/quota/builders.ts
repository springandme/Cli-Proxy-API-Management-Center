...
import {
  ...
  GEMINI_CLI_IGNORED_MODEL_PREFIXES,
} from './constants';
...
export function buildGeminiCliQuotaBuckets(
  ...
  const grouped = new Map<string, GeminiCliQuotaBucketState>();
  const ungrouped: GeminiCliQuotaBucketState[] = [];

  buckets.forEach((bucket) => {
    if (GEMINI_CLI_IGNORED_MODEL_PREFIXES.some((prefix) => bucket.modelId.startsWith(prefix))) {
      return;
    }
    ...
    existing.modelIds = Array.from(new Set([...(existing.modelIds ?? []), bucket.modelId]));

    if (
      bucket.remainingFraction !== null &&
      (existing.remainingFraction === null || bucket.remainingFraction < existing.remainingFraction)
    ) {
      existing.remainingFraction = bucket.remainingFraction;
      existing.remainingAmount = bucket.remainingAmount;
      existing.resetTime = bucket.resetTime;
      existing.tokenType = bucket.tokenType;
    }
  });

  const groupedBuckets = Array.from(grouped.values()).sort((a, b) => {
    const orderA = GEMINI_CLI_GROUP_ORDER.get(a.id) ?? Number.MAX_SAFE_INTEGER;
    const orderB = GEMINI_CLI_GROUP_ORDER.get(b.id) ?? Number.MAX_SAFE_INTEGER;
    if (orderA !== orderB) return orderA - orderB;
    return a.label.localeCompare(b.label);
  });

  ungrouped.sort((a, b) => a.label.localeCompare(b.label));
  return [...groupedBuckets, ...ungrouped];
}
